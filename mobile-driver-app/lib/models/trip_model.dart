...
// Paperwork uploaded at either end of the trip - pickup-point documents (LR
// copy, invoice, gate pass, etc., see order.model.js's pickupDocuments) and
// proof-of-delivery documents (signed receipt, POD photo, etc., see
// deliveryDocuments) share this exact shape, so one class covers both - see
// driver.controller.js's uploadPickupDocuments/uploadDeliveryDocuments. Any
// file format, so originalName is the only hint the UI has for choosing an
// icon/label; url is server-relative (`/uploads/...`), same as other upload
// fields in this app.
class TripDocument {
  final String url;
  ...
  TripDocument({required this.url, this.originalName, this.uploadedAt});
...
class TripModel {
  ...
  final List<TripDocument> pickupDocuments;
  final List<TripDocument> deliveryDocuments;
  ...
  TripModel({
    ...
    this.deliveryDocuments = const [],
  });

  TripModel copyWith({
    String? status,
    List<TripDocument>? pickupDocuments,
    List<TripDocument>? deliveryDocuments,
  }) {
    return TripModel(
      id: id,
      ...
      deliveryDocuments: deliveryDocuments ?? this.deliveryDocuments,
    );
  }
  ...
  factory TripModel.fromJson(Map<String, dynamic> json) {
    ...
    return TripModel(
      ...
      pickupDocuments: (json['pickupDocuments'] as List<dynamic>? ?? [])
          ...
          .toList(),
      deliveryDocuments: (json['deliveryDocuments'] as List<dynamic>? ?? [])
          .map((d) => TripDocument.fromJson(d as Map<String, dynamic>))
          .toList(),
    );
  }
